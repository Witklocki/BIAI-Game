...
def play():
    ...
    if game_runner.play(int(x), int(y)):
        ...
        status = 'Fail'
    return json.dumps({'status': status, 'game_status': game_runner.getStatus()})
        ...
        return json.dumps(
            {
                ...
                'game_status': game_runner.getStatus()
            }
        )
    return json.dumps({'status': 'Fail', 'game_status': game_runner.getStatus()})


@app.route('/get_gamestate')
def get_gamestate():
    return json.dumps({'status': 'OK', 'game_status': game_runner.getStatus()})
