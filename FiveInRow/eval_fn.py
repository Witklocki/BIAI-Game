...
def statusCheck(state, current_color):
    return color(state, piece.BLACK, current_color) + \
        color(state, piece.WHITE, current_color)


def color(state, color, current_color):
    values = state.values
    size = state.size
    ...
    # check rows and cols
    for i in range(size):
        evaluation += chceckLine(values[i, :], color, current)
        evaluation += chceckLine(values[:, i], color, current)

    # check diags
    for i in range(-size + 5, size - 4):
        evaluation += chceckLine(np.diag(values, k=i),
                                 color,
                                 current)
        evaluation += chceckLine(np.diag(np.fliplr(values), k=i),
                                 color,
                                 current)
...
def chceckLine(line, color, current):
    evaluation = 0
    size = len(line)
    # consecutive
    consec = 0
